...
VRPMetric {
	...
	*configureCVDcolors { arg mapFile=nil, mapFileDiscrete=nil, colorSetFile=nil;
		...
		FonaDyn.setPaths;
		ColorMap.setDefaultPath(~fdExtensions +/+ "FonaDynTools/colormaps");
	...
	installColorMapping { arg mapFile, minCol, maxCol, minValue, maxValue;
		...
		if (nColors <= 0, {
			...
			if (minCol.notNil,   { minColor = minCol.asFloat });
			if (maxCol.notNil,   { maxColor = maxCol.asFloat });
			if (minValue.notNil, { minVal = minValue.asFloat });
			if (maxValue.notNil, { maxVal = maxValue.asFloat });
		});
		^retVal
	}
